    ...
    override fun run() {
        ...
        if (!postedSuccessfully) {
            Logger.privateI(
                LogAspect.ANR,
                TAG,
                { "ANR detection operation cannot be posted to UI as UI thread is possibly shutting down" })
            return
        }
        ...
        val success: Boolean = try {
            ...
            Logger.privateW(
                LogAspect.ANR,
                TAG,
                { "ANR Detector was interrupted with exception ${e.message}" })
            return
        }
        ...
        if (anrCounter.incrementAndGet() >= thresholdSeconds) {
            ...
            if (!ignoreDebugger && (Debug.isDebuggerConnected() || Debug.waitingForDebugger())) {
                Logger.privateW(
                    LogAspect.ANR,
                    TAG,
                    { "An ANR was detected but ignored because the debugger is connected (you can prevent this by changing ignoreDebugger to true)" })
                return
            } else {
                anrCounter.set(0)
