...
class SplunkMaskBodyInterceptor(
    ...
    override fun onIntercept(original: SplunkChain, intercepted: SplunkNetworkRequest): SplunkNetworkRequest {
        if (intercepted.requestBody == null && intercepted.responseBody == null)
            return intercepted

        masks.forEachFast { mask ->
            intercepted.requestBody = intercepted.requestBody?.replace(mask.regex, mask.replaceWith)
            intercepted.responseBody = intercepted.responseBody?.replace(mask.regex, mask.replaceWith)
        }
