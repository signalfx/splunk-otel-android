...
/**
 * A SpanExporter that is configured to modify some of its attributes
 * at export time.
 */
public class AttributeModifyingSpanExporter implements SpanExporter {
    ...
    public AttributeModifyingSpanExporter(SpanExporter delegate, Map<AttributeKey<?>, Function<?, ?>> spanAttributeReplacements) {
    ...
    public CompletableResultCode export(Collection<SpanData> spans) {
        ...
        List<SpanData> modifiedSpans = spans.stream().map(this::doModify).collect(Collectors.toList());
    ...
    private Attributes buildModifiedAttributes(SpanData span) {
        ...
        span.getAttributes().forEach((key,attrValue) -> {
